...
@if (session('success'))
    <div id="alert-banner-success"
        class="fixed top-4 left-1/2 transform -translate-x-1/2 z-[60] <!-- Changed from z-50 to z-[60] -->
               max-w-lg w-full  rounded-lg shadow-md flex items-center justify-between relative
...
@if (session('error'))
    <div id="alert-banner-error"
        ...
        <button class="p-1 rounded-full border border-gray-400 text-gray-600 hover:bg-gray-100 hover:border-gray-600 absolute top-2 right-2 transition"
            onclick="closeBanner(this)">
            <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" fill="none" <!-- Fixed: changed w-2 h-2 to w-4 h-4 -->
                viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
            </svg>
        </button>
